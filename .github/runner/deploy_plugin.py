...
def marketplace_row(*, create: bool) -> dict[str, Any]:
    ...
    if row is None and create:
        ...
        return marketplace_row(create=False)
...
def load_state() -> dict[str, Any] | None:
    ...
    try:
        ...
        raise DeployError("deployment receipt is unreadable") from exc
    if not isinstance(value, dict) or value.get("schema") != "bears-plugin-deploy-state.v1":
        raise DeployError("deployment receipt has an invalid schema")
    return value
...
def promote(requested: str) -> str:
    ...
    if state is not None:
        ...
        if not SHA_RE.fullmatch(current) or not VERSION_RE.fullmatch(current_version):
            raise DeployError("deployment receipt identity is invalid")
...
def main() -> int:
    ...
    descriptor = os.open(LOCK_FILE, os.O_CREAT | os.O_RDWR | os.O_CLOEXEC | os.O_NOFOLLOW, 0o600)
    try:
        fcntl.flock(descriptor, fcntl.LOCK_EX)
        status = promote(sys.argv[1])
