...
- Do not add autoCD controller code, local candidate producers, marketplace-cache refresh code, or deployment fallbacks. After one-time local marketplace bootstrap, the external controller invokes this repo's explicit installer only after its same-SHA local autoCI `PASS` and cache refresh.
