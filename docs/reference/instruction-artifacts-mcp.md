...
- For catalog JSON surfaces, weak-term scoring parses JSON and reads only selected human-policy string fields such as `description`, `rule`, `enforcement`, `decision`, `rationale`, `scope`, `trust_boundary`, `allowed_write_boundary`, and `required_precision`. JSON keys, identifiers, paths, commands, required-validation entries, and other machine metadata are not scored as instruction prose.
