...
7. **Emit the exact blocker packet** — emit the exact `ROLE_COVERAGE_BLOCKER` packet when coverage is unknown, unmapped, parent-only, missing, broad, or ambiguous.
8. **Require mandatory validator/test coverage** — prove one-primary-role routing, blocker behavior, reviewer-only attachments, and implementation-handoff blocking before edits continue.
...
Emit the exact packet defined in `assets/catalog/role-gate-methodology.v1.json` and emitted by `scripts/subagents_roles.py route <target>`.
...
- `python3 scripts/role_gate_methodology.py validate`; manual execution requires one exact operator-named command.
- `python3 scripts/subagents_roles.py validate`; manual execution requires one exact operator-named command.
- `python3 -m unittest tests/test_role_gate_methodology.py tests/test_subagents_roles.py`; manual execution requires one exact operator-named command.
