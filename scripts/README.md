...
`app_graph_engine.py` remains slightly above 400 lines so loading, legacy normalization, edge-registry checks, traversal direction, cycle detection, and query bounds share one invariant surface. The repository has no active autoCI/CD workflow; these modules add no agent-executable test or acceptance command, and acceptance remains `not_run` unless authentic external autoCI evidence is supplied.
