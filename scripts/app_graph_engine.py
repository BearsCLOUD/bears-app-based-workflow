...
from app_graph_handoff import handoff_validate
...
READ_TOOLS = {"dependency_slice","impact_analysis","graph_trace","graph_diagnostics","topological_plan","workflow_state","handoff_validate"}
...
def execute_tool(name: str, arguments: dict[str, Any], *, maintainer: bool = False) -> dict[str, Any]:
    ...
    if name == "handoff_validate": return handoff_validate(arguments)
    if name not in READ_TOOLS: raise GraphError("METHOD_NOT_FOUND", "unknown read-only tool")
    return execute_query(name, arguments)


__all__ = ["GraphError","GraphStore","MAX_REQUEST_BYTES","MAX_RESPONSE_BYTES","execute_tool","graph_compile","handoff_validate","process_record_event"]
