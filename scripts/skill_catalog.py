...
def validate_active_skill_route_coverage(
    ...
    if not catalog.get("active_skills"):
        return []
...
def validate_catalog(catalog: dict[str, Any], plugin_root: Path = PLUGIN_ROOT) -> list[str]:
    ...
    if policy.get("plugin_instruction_source") != "disabled":
        errors.append("discovery_policy.plugin_instruction_source must be disabled")
    if policy.get("manifest_skill_root") is not None:
        errors.append("discovery_policy.manifest_skill_root must be null when plugin skills are disabled")
    ...
    if active:
        errors.append("active plugin skills are forbidden; plugin is not an instruction source")
...
def render_spec_fragment(catalog: dict[str, Any]) -> str:
    ...
    active_names = ", ".join(f"`{entry['name']}`" for entry in catalog["active_skills"]) or "None"
    lines = [
        "<!-- generated by scripts/skill_catalog.py; edit assets/catalog/plugin-skill-catalog.v1.json -->",
