#!/usr/bin/env python3
"""Validate, route, and audit Bears plugin-owned subagents roles."""
...
ROLE_DEVELOPMENT_OWNER_ROLE = "bears-subagents-roles-governor"
...
VALIDATION_EXECUTION_POLICY = (
    "Only subagents_roles.py route/audit gates are agent-local. Other required checks are "
...
def _is_agent_local_route_audit_gate(command: str) -> bool:
    ...
    return (
        tokens[0] == "python3"
        and tokens[1].endswith("scripts/subagents_roles.py")
...
def _validate_policy(policy: dict[str, Any], errors: list[str]) -> None:
    ...
    if not isinstance(role_development, dict):
        ...
        if role_development.get("owner_role") != ROLE_DEVELOPMENT_OWNER_ROLE:
            errors.append("mandatory_policy.role_development.owner_role must be bears-subagents-roles-governor")
    ...
    if not isinstance(audit, dict):
        ...
        if audit.get("auditor_role") != "bears-subagents-roles-governor":
            errors.append("mandatory_policy.independent_control_audit.auditor_role must be bears-subagents-roles-governor")
...
def audit_target(catalog: dict[str, Any], target: str, *, plugin_root: Path = PLUGIN_ROOT) -> dict[str, Any]:
    ...
    audit_packet = {
        "auditor_role": audit_policy.get("auditor_role", "bears-subagents-roles-governor"),
...
LEDGER_PATH = PLUGIN_ROOT / "docs/audits/subagents-roles/role-principle-ledger.v1.json"
LEDGER_SCHEMA = "bears-role-principle-ledger.v1"
PRINCIPLE_REFERENCES = {
    "well_specified_agent_artifacts": "skills/subagents-roles/references/well-specified-artifacts.md",
    "runtime_result_efficiency": "skills/subagents-roles/references/runtime-result-efficiency.md",
    "repeatable_process_automation": "skills/subagents-roles/references/repeatable-process-automation.md",
}
...
def _role_catalog_entry(catalog: dict[str, Any], agent_file: str) -> dict[str, Any] | None:
    for role in catalog.get("roles", []):
        if isinstance(role, dict) and role.get("agent_file") == agent_file:
            return role
    return None


def build_role_principle_ledger(catalog: dict[str, Any], plugin_root: Path = PLUGIN_ROOT) -> dict[str, Any]:
    """Build the tracked role-principle ledger from role TOML and catalog metadata."""
    ...
    for path in sorted((plugin_root / "agents").glob("*.toml")):
        ...
        try:
            ...
            profile = {"name": path.stem, "_load_error": str(exc)}
        role_name = str(profile.get("name") or path.stem)
        catalog_role = _role_catalog_entry(catalog, rel_path)
        executable_owner = "@Bears role/skill/subagent flow"
        executable_consumer = "codex subagent profile"
        if catalog_role:
            executable_owner = str(catalog_role.get("name", role_name))
            executable_consumer = str(catalog_role.get("execution_class") or "codex subagent profile")
        for principle_id, reference in PRINCIPLE_REFERENCES.items():
            entries.append({
                "principle_id": principle_id,
                "role_profile": role_name,
                "target_files": [rel_path],
                "executable_consumer": executable_consumer,
                "executable_owner": executable_owner,
                "status": "covered" if catalog_role and "_load_error" not in profile else "review_required",
    ...
    return {
        ...
        "updated": utc_now(),
        "principles": [
            {"id": key, "reference": value, "ledger_fields": ["role_profile", "target_files", "executable_consumer", "executable_owner", "status", "evidence_ref", "issue_ref"]}
            for key, value in PRINCIPLE_REFERENCES.items()
        ],
        "entries": entries,
    }


def audit_role_principle_ledger(ledger: dict[str, Any], plugin_root: Path = PLUGIN_ROOT) -> list[str]:
    """Return deterministic role-principle ledger coverage errors."""
    ...
    if not isinstance(entries, list) or not entries:
        ...
        return errors
    existing_agents = {_rel(path) for path in sorted((plugin_root / "agents").glob("*.toml"))}
    by_file: dict[str, set[str]] = {path: set() for path in existing_agents}
    required_principles = set(PRINCIPLE_REFERENCES)
    for index, entry in enumerate(entries):
        if not isinstance(entry, dict):
            errors.append(f"entries[{index}] must be an object")
            continue
        principle = entry.get("principle_id")
        if principle not in required_principles:
            errors.append(f"entries[{index}].principle_id unknown: {principle}")
        target_files = entry.get("target_files")
        if not isinstance(target_files, list) or not target_files:
            errors.append(f"entries[{index}].target_files must be a non-empty list")
            continue
        for target in target_files:
            if not isinstance(target, str) or target not in existing_agents:
                errors.append(f"entries[{index}].target_files contains non-agent path: {target}")
                continue
            by_file[target].add(str(principle))
        for field in ("role_profile", "executable_consumer", "executable_owner", "status", "evidence_ref", "updated"):
            if not isinstance(entry.get(field), str) or not entry[field].strip():
                errors.append(f"entries[{index}].{field} must be a non-empty string")
        if entry.get("status") not in {"covered", "review_required", "issue_open"}:
            errors.append(f"entries[{index}].status must be covered, review_required, or issue_open")
    for target, principles in sorted(by_file.items()):
        missing = required_principles - principles
        if missing:
            errors.append(f"missing principle coverage for {target}: {sorted(missing)}")
    return errors
...
def ledger_summary_payload(ledger: dict[str, Any]) -> dict[str, Any]:
    ...
    for entry in entries:
        ...
        principles[str(entry.get("principle_id"))] = principles.get(str(entry.get("principle_id")), 0) + 1
    return {
        "schema": "bears-role-principle-ledger-summary.v1",
        "ledger_schema": ledger.get("schema"),
        "updated": ledger.get("updated"),
        "entry_count": len(entries),
        "status_counts": statuses,
        "principle_counts": principles,
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ledger_refresh = sub.add_parser("ledger-refresh", help="Refresh the role-principle ledger artifact")
    ledger_refresh.add_argument("--path", default=str(LEDGER_PATH))
    ledger_audit = sub.add_parser("ledger-audit", help="Audit the role-principle ledger artifact")
    ledger_audit.add_argument("--path", default=str(LEDGER_PATH))
    ledger_summary = sub.add_parser("ledger-summary", help="Print compact role-principle ledger summary")
    ledger_summary.add_argument("--path", default=str(LEDGER_PATH))
    ledger_summary.add_argument("--json", action="store_true")
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "validate":
        ...
        if errors:
            ...
            return 1
        print(f"subagents roles catalog ok: {args.catalog}")
        return 0
    ...
    if args.command == "ledger-refresh":
        ledger = build_role_principle_ledger(catalog)
        path = Path(args.path)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps(ledger, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        print(f"role-principle ledger updated: {path}")
        return 0
    ...
    if args.command == "ledger-audit":
        try:
            ...
            print(f"ERROR: {exc}", file=sys.stderr)
            return 1
        errors = audit_role_principle_ledger(ledger)
        if errors:
            for error in errors:
                print(f"ERROR: {error}", file=sys.stderr)
            return 1
        print(f"role-principle ledger ok: {args.path}")
        return 0
