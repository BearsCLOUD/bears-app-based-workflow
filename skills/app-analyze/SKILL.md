...
- Read the exact source, decision, requirement, functional-map, ledger, artifact, evidence, result, review, remediation, process-index, and handoff refs named by that build.
...
Bind `analysis_ref`, profile, model, checklist, exact basis build, every consumed input ref, complete coverage counts, findings, unmapped decisions, unmapped requirements, open remediation refs, completeness, and route.
...
6. Validate the candidate handoff and transition against workflow v3 and handoff v4.
7. Record one actual native v3 analysis event with `analysis_ref` through the `DIRECT` primary or repo-L2.
8. Reconcile the resulting journal and emit the build-bound terminal or corrective handoff.
