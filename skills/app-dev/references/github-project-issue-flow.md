...
route_targets=<paths requiring computed owner coverage>
...
3. Require `$app-functional-graph` evidence before wave grouping; do not run graph or ledger validators unless the operator names the exact command.
...
7. Record `computed_owner_role` and `expected_autoci_status` for every target path; do not run route/audit unless the operator names the exact command.
...
computed_owner_role=<route-selected @Bears role>
expected_autoci_status=<status name or none>
...
Use this packet only when L2 must pass a tiny L3 slice through a constrained runtime proxy after a failed or timed-out run. The packet must keep owner coverage intact and must not widen the worker scope.

```text
mini_runtime_proxy=true
proxy_lane=l3
proxy_reason=<exact missing or constrained runtime capability>
route_selected_role=<computed/selected @Bears owner role>
...
metadata_mutation_authorized=<true|false>
first_minute_progress_proof=<READY|WIP|FAST_BLOCKER packet id or exact proof text; proxy recovery only>
first_minute_proof_time=<UTC timestamp or elapsed seconds from assignment receipt; proxy recovery only>
execution_lane=tiny_one_file_or_one_path
post_fix_no_wip_gate=<exact command proving task-owned WIP is absent after PASS; proxy recovery only>
...
- In proxy recovery mode, write work may continue only after `first_minute_progress_proof` exists. Missing proof returns `FAST_BLOCKER` with no writes and no L3 dispatch.
- After a timeout, failed proxy run, or missing worker-authority proof, use only one tiny execution lane until post-fix completion evidence plus `post_fix_no_wip_gate` proves no task-owned WIP.
...
When owner coverage returns `ROLE_COVERAGE_BLOCKER`, a selected role lacks exact write scope, or role text grants forbidden implementation authority, L2 records a blocker and returns it to the parent. Do not spawn L3 for role edits from app-dev.
