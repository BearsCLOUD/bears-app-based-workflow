...
Start with `$app-context-index`. Keep one route state containing the current canonical handoff, source digest, semantic fingerprint, and the answer or evidence refs added since it was produced. Refresh the index only when the digest changes or a stage emits `needs-index`.

## Canonical routes

Validate every supplied or stage-produced `app-stage-handoff.v2` against `contracts/app-stage-handoff.v2.schema.json`. Resolve every status/target pair only from `contracts/app-workflow-definition.v1.json`; never retain or reconstruct a local route table.
...
Never infer completion only from a file's presence. Use its stage status, required refs, coverage, current source digest, and context index result.
...
The semantic fingerprint covers the complete canonical handoff, workflow definition digest, and source snapshot digest after stable key ordering. Never execute the same target again from an unchanged fingerprint.
...
Return the last validated `app-stage-handoff.v2` when one exists; otherwise return the `app-constitution` boundary transition. Also return the last context-index result, last executed internal stage, and one stop reason: `app-constitution-boundary`, `app-dev-boundary`, `pass`, `blocked`, `unchanged-waiting`, `unchanged-handoff`, `user-architecture-decision`, or `invalid-handoff`.
