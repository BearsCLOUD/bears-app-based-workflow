...
description: Evaluate and execute requested semantic role taxonomy creation, merge, split, or deletion using a repeatable comparison and least-privilege method.
---
...
This skill supplies the comparison method and packet templates only for a requested semantic role taxonomy operation: `create`, `merge`, `split`, or `delete`. The invoking role owns the operation decision, permission decision, profile content, acceptance decision, edits, and delivered result. `subagents` owns role selection and dispatch.
...
5. Evaluate the requested taxonomy change and choose one result using these criteria: `reuse` when the requested change is unnecessary because an existing profile already owns the deliverable and permission boundary; `refactor` when the requested taxonomy change instead requires an in-place semantic correction; `merge` when duplicate profiles have the same deliverable and permission boundary; `split` when one profile contains multiple deliverables or permission boundaries; `create` when no existing profile owns the deliverable; `reject` when the request has no objective deliverable, conflicts with authority, or cannot have a safe permission boundary; `delete` when a profile has no remaining deliverable or its ownership moved to an accepted profile.
