...
Use this skill when an agent must create a Bears-approved generated value and place it in Infisical through the plugin-owned write-only path.
...
7. For local generated values, run:
   `python3 /srv/bears/plugins/bears/scripts/secret_factory.py create <request.json>`
8. For preflight without generation or write, run:
   `python3 /srv/bears/plugins/bears/scripts/secret_factory.py create <request.json> --dry-run`
9. For provider-owned values, return the handoff packet from:
   `python3 /srv/bears/plugins/bears/scripts/secret_factory.py plan <request.json>`
10. For cataloged provider-owned Infisical refs, return names-only readiness from:
   `python3 /srv/bears/plugins/bears/scripts/secret_factory.py handoff-readiness`
...
12. For refs with `existence_status=operator_confirmed_live_ref`, use the exact `provider_api_routing` entry from `assets/catalog/secret-factory.v1.json`; resolve the token only at task time and never print it.
...
- Do not replace the Bears GitLab API route; use scheme `https`, host `bears.gitlab.yandexcloud.net`, and path prefix `/api/v4` for the `gitlab` route. Never use host `gitlab.com`.
- Do not use `list_secrets` for documented-unconfirmed ref confirmation unless the MCP surface has a names-only or exclude-values mode.
...
- Local commit validation owns `python3 scripts/validate_overlay.py --json scan-static-safety --path <repo-relative-file>` for changed files that can carry static endpoint, URI, IP, CIDR, inventory, or live-tool wording; manual execution requires operator approval.
