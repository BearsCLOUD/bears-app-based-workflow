...
For Bears target-native work that needs durable project rules, functional documentation, GitHub Projects planning, or execution from Project items, use the Bears target skill chain: `$app-constitution` -> `$app-specify` -> `$github-project-planning` when Project fields/views are needed -> `$app-plan` -> `$app-analyze` -> `$app-dev`.
...
11. `$speckit-analyze` from `/srv/bears/.agents/skills` to detect cross-artifact drift; implementation stays blocked until it passes or the operator explicitly rescope-fixes the packet. For GitHub Project-backed Bears plans, run `$app-analyze` on the constitution, spec, docs, Project items, Issues, route/audit roles, validation, dependencies, and `$app-dev` handoff.
12. **Spec Kit gate**: require `spec.md`, `plan.md`, `tasks.md`, and analyze PASS, or for Bears target-native flow require constitution packet, specification packet, GitHub Project task packet, and `$app-analyze` PASS.
13. **Role gate**: prove each selected `tasks.md` item or GitHub Project item matches the exact Bears specialist role.
14. **Subagent execution**: the main agent orchestrates; subagents execute concrete tasks from `tasks.md` or `$app-dev` executes ready GitHub Project items. Tasks marked `[P]` or disjoint Project items should run in parallel when paths are disjoint and role coverage exists.
15. `$speckit-implement` from `/srv/bears/.agents/skills` or `$app-dev` only after the implementation scope is approved.
...
1. `$app-constitution` records project rules, artifact owners, forbidden actions, validation duties, and amendment rules.
2. `$app-specify` creates or updates functional requirements and user/operator/developer documentation from the constitution and repo evidence.
...
4. `$app-plan` converts app docs into concrete GitHub Issues, Project #20 items, dependencies, route-selected roles, validation, and mini-model-ready L3 goal blocks.
5. `$app-analyze` checks constitution, spec, docs, Project plan, Issues, route/audit roles, validation, dependencies, and execution handoff for drift.
6. `$app-dev` executes only after analysis returns `pass` or the operator explicitly approves a scoped execution with listed advisory findings.

Do not let `$app-dev` create the Project planning model. It consumes the Project/Issue state created by `$app-plan`.
