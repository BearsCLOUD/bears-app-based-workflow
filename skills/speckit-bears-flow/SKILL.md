...
11. `$speckit-analyze` from `/srv/bears/.agents/skills` to detect cross-artifact drift; implementation stays blocked until it passes or the operator explicitly rescope-fixes the packet.
12. **Spec Kit gate**: require `spec.md`, `plan.md`, `tasks.md`, and analyze PASS.
13. **Role gate**: prove each selected `tasks.md` item matches the exact Bears specialist role.
14. **Subagent execution**: the main agent orchestrates; subagents execute concrete tasks from `tasks.md`. Tasks marked `[P]` should run in parallel when paths are disjoint and role coverage exists.
15. `$speckit-implement` from `/srv/bears/.agents/skills` only after the implementation scope is approved.
