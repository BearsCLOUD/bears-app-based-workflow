...
`gate` means an automatic admission decision. `automatic status` means an automatic verification line. `autoCD` means an automatic install, update, or deploy line.
...
1. Read `the @Bears plugin checkout/AGENTS.md`, `assets/catalog/platform-role-catalog.v1.json`, `assets/catalog/role-gate-methodology.v1.json`, and the target router.
...
4. Leave route/audit and ledger proof to `automatic status` or local commit validation.
...
- `the dev workspace` routes to workspace governance only.
- `the dev workspace/control`, `the platform checkout`, `the dev workspace/products`, `the dev workspace/quality`, `the dev workspace/infrastructure`, and `the dev workspace/ops` are group classifiers; route a narrower concrete layer for implementation.
- `kube`, `kubernetes`, `bears-infra`, and `the environment-control checkout` route to the Kubernetes repo boundary.
- `the apps checkout` and `the apps repository` route to the product-app monorepo root.
