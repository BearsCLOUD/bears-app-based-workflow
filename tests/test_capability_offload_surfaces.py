...
FIXTURES = PLUGIN_ROOT / "tests/fixtures/capability_layout/offload_surfaces"
...
class CapabilityOffloadSurfaceTests(unittest.TestCase):
    def run_layout(self, packet):
        result = subprocess.run(
            [sys.executable, str(SCRIPT), "validate-offload-claims", "--packet", str(packet), "--snapshot", str(SNAPSHOT), "--json"],
            cwd=PLUGIN_ROOT,
            text=True,
    ...
    def test_pass_fixture_validates_all_offload_surfaces(self):
        result, payload = self.run_layout(PASS_PACKET)
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(payload["status"], "pass")
        self.assertEqual(payload["claim_count"], 8)
        self.assertEqual(payload["surface_count"], 8)
