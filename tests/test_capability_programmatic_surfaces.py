...
FIXTURES = PLUGIN_ROOT / "tests/fixtures/capability_layout/programmatic_surfaces"
...
class CapabilityProgrammaticSurfaceTests(unittest.TestCase):
    def run_layout(self, packet):
        result = subprocess.run(
            [sys.executable, str(SCRIPT), "validate-programmatic-surfaces", "--packet", str(packet), "--snapshot", str(SNAPSHOT), "--json"],
            cwd=PLUGIN_ROOT,
            text=True,
    ...
    def test_pass_fixture_validates_all_programmatic_surfaces(self):
